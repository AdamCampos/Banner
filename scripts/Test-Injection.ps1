...
function Get-RelevantLogPath {
    param([string]$Path)
    $dir  = [IO.Path]::GetDirectoryName($Path)
    $name = [IO.Path]::GetFileNameWithoutExtension($Path)
    $ext  = [IO.Path]::GetExtension($Path)
    if ([string]::IsNullOrEmpty($ext)) { $ext = ".log" }
    return (Join-Path $dir ($name + ".relevant" + $ext))
}
...
Add-Content -Path $VbsPath -Value @'
Set sh = CreateObject("WScript.Shell")
sh.Environment("PROCESS")("FTA_LOG_RELEVANT") = "1"  ' <--- habilita RelevantFileSink no processo Smoke

WScript.Sleep 4000
Set d = CreateObject("Scripting.Dictionary")
d.Add "k1", "v1"
Set sh2 = CreateObject("WScript.Shell")
sh2.Environment("PROCESS")("ABC") = "123"
For i = 1 To 3
...
$rel = Get-RelevantLogPath -Path $LogPath
if (Test-Path $rel) {
    Write-Host "---- Conteúdo do relevante: $rel"
    Get-Content -LiteralPath $rel -Tail 30
}
}
}
